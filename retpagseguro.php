...
if('POST' == $method){
    ...
    if ($type === 'transaction'){
        ...
        try {
            ...
            $pagamento->insertPagSeguro($arrayPDO['name'], $arrayPDO['item_count'], $arrayPDO['transaction_code']);
            ...
            try {
                ...
                if ($arrayPDO['status'] == 'PAID') {
					...
					$stmt = $conn->prepare('UPDATE accounts SET backup_points = backup_points + :item_count WHERE name = :name');
                    $stmt->execute(array('item_count' => $arrayPDO['item_count'], 'name' => $arrayPDO['name']));
