...
if($config['site']['shop_system'] == 1) {
	...
	function getOfferArray() {
        ...
        if (isset($_GET['filtro'])) {
            $filtro_where = " WHERE type = '{$_GET['filtro']}' ";
        }
		$offer_list = $GLOBALS['SQL']->query('SELECT * FROM '.$GLOBALS['SQL']->tableName('z_shop_offer').' '.$filtro_where.' ;');
		$i_pacc = 0;
		$i_item = 0;
		$i_vipdays = 0;
		$i_itemvip = 0;
...
if($player_vip_time > 0){
	$newVipDays = $player_vip_time + ($buy_offer['days']);
	$merda = $SQL->query("UPDATE `accounts` SET `premdays` = '$newVipDays' WHERE `name` = '$nomedoComprador'");
}else{
	$newVipDays2 = ($buy_offer['days']);
	$merda = $SQL->query("UPDATE `accounts` SET `premdays` = '$newVipDays2' WHERE `name` = '$nomedoComprador'");
...
$main_content .='<br /><center><div class="error"><b>You do not have premium points available.</b><br /><br /><form action="index.php?subtopic=accountmanagement" method="post"><input type="submit" value="Buy now Premium Points here!" class="btn btn-success" /></form></div></center>'; 
else
